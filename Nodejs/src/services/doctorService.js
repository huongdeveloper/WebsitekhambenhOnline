...
let checkRequiredFields = (inputData) => {
    let arrFields = ['doctorId', 'contentHTML', 'contentMarkdown', 'action', 'selectedPayment', 'selectedPrice',
...
let saveDetailInforDoctor = (inputData) => {
    return new Promise(async (resolve, reject) => {
        try{
            let chechObj = checkRequiredFields(inputData);
            if(chechObj.isValid === false) {
                resolve({
                    errCode: 1,
                    errMessage: `Missing parameter: ${chechObj.element}`
                })
            }else {
    ...
    return new Promise(async (resolve, reject) => {
        try{
            if(!doctorId || !date) {
                ...
                let dateSchedule = await db.Schedule.findAll({
                    where: {
                        doctorId: doctorId,
                        date: date
                    },
